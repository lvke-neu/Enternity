...
namespace Enternity
{
	...
	Mesh* MeshProvider::getMeshSync(const char* fullPath)
	{
		...
		if (meshAsset->getLoadingState() == Asset2::LoadingState::loading_state_succeeded)
		{
	...
	Mesh* MeshProvider::getMeshSync(BasicMeshType type, const std::string& texturePath)
	{
		...
		if (basicMeshAsset->getLoadingState() == Asset2::LoadingState::loading_state_succeeded)
		{
	...
	void MeshProvider::tick(void* data)
	{
		...
		{
			if (it->meshAsset->getLoadingState() == Asset2::LoadingState::loading_state_succeeded)
			{
