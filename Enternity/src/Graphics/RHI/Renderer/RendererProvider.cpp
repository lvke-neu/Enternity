...
namespace Enternity
{
	...
	Renderer* RendererProvider::getRendererSync(const char* vsFullPath, const char* psFullPath)
	{
		...
		if (vsRendererAsset->getLoadingState() == BlobLoader::LoadingState::loading_state_succeeded &&
			psRendererAsset->getLoadingState() == BlobLoader::LoadingState::loading_state_succeeded)
		{
	...
	void RendererProvider::tick(void* data)
	{
		...
		{
			if (it->vsRendererAsset->getLoadingState() == BlobLoader::LoadingState::loading_state_succeeded
				&& it->psRendererAsset->getLoadingState() == BlobLoader::LoadingState::loading_state_succeeded)
			{
