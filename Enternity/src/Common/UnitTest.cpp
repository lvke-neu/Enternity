...
int main(int argc, const char** argv) {
	...
	std::string path = "mesh://primitive=planea";
	auto mesh = Engine::GetInstance().getAssetLoader()->getAsset(path.c_str());

	if (!mesh->isLoadSucceeded())
	{
		mesh = Engine::GetInstance().getAssetLoader()->getAsset("mesh://primitive=plane");
		mesh->unload();
